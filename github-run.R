...
userRecommendation = 	function(userID, data.df) {
							...
							user.assoc.df = final.df[ rev(order(final.df$count)), ]
							append(user.assoc, sort(user.assoc.df[1:10, 1]))
							...
							# Order the counts so that the most frequent appear at the top
							user.fork.assoc.df = final.df[ rev(order(final.df$count)), ]
							append(user.fork.assoc, sort(user.fork.assoc.df[1:10, 1]))
							...
							for(lang in as.character(unique(currentUserLangs$language))) {
								...
								subset.sum = sum(current.subset$lines)
								subset.length = length(current.subset$lines)
								calculatedProportion = subset.sum / subset.length
								
								# Appending the language to userLanguageWeightings
								rbind(userLanguageWeightings, data.frame(language=lang, lines=calculatedProportion))
							}
							...
							for(repo in final.df$repoID) {
								...
								diff = accumulator / langAmount
								finalRepoLangWeighting = 1 - diff
...
allRecommendations =	function(listOfUsers) {
							...
							user.assocTxt = paste(paste(user, ":", sep=""), paste(user.assoc, sep=",", collapse=","), sep="")
							user.fork.assocTxt = paste(paste(user, ":", sep=""), paste(user.fork.assoc, sep=",", collapse=","), sep="")
							
							write(user.assocTxt, file="results-user.txt")
							write(user.fork.assocTxt, file="results-user-fork.txt")
							write(results, file="results-user-fork-lang.txt")
						}
